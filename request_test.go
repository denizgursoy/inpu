...
func (c *ClientSuite) Test_Multiple_Chose_Correct_Reply_Behaviour() {
	...
	c.T().Run("should select StatusIs over StatusIsSuccess", func(t *testing.T) {
		err := Get(server.URL).
			QueryString("foo", "bar1").
			QueryString("foo", "bar2").
			On(StatusIsSuccess, ThenReturnError(errors.New("unexpected status"))).
			On(StatusIs(http.StatusOK), ThenReturnError(expectedError)).
			Send()
	...
	c.T().Run("should select StatusIs over StatusIsOneOf", func(t *testing.T) {
		err := Get(server.URL).
			QueryString("foo", "bar1").
			QueryString("foo", "bar2").
			On(StatusIsOneOf(http.StatusOK, http.StatusAccepted), ThenReturnError(errors.New("unexpected status"))).
			On(StatusIs(http.StatusOK), ThenReturnError(expectedError)).
			Send()
	...
	c.T().Run("should select StatusIsOneOf over StatusIsSuccess", func(t *testing.T) {
		err := Get(server.URL).
			...
			QueryString("foo", "bar2").
			On(StatusIsSuccess, ThenReturnError(errors.New("unexpected status"))).
			On(StatusIsOneOf(http.StatusOK, http.StatusAccepted), ThenReturnError(expectedError)).
			Send()
	...
	c.T().Run("should select StatusIsOneOf over StatusAny ", func(t *testing.T) {
		err := Get(server.URL).
			QueryString("foo", "bar1").
			QueryString("foo", "bar2").
			On(StatusAny, ThenReturnError(errors.New("unexpected status"))).
			On(StatusIsOneOf(http.StatusOK, http.StatusAccepted), ThenReturnError(expectedError)).
			Send()
