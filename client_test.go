...
func (c *ClientSuite) Test_Client() {
	...
	err = client.Post(server.URL, BodyJson(testData)).
		On(StatusAnyExcept(http.StatusCreated), ThenReturnError(errors.New("unexpected status"))).
		Send()
...
func (c *ClientSuite) Test_Client_No_Redirect() {
	...
	err := New().
		...
		On(StatusIsRedirection, ThenReturnError(redirectionError)).
...
func (c *ClientSuite) Test_Client_Cookie() {
	...
	err = client.Get(server.URL+"/check").
		On(StatusAnyExcept(http.StatusOK), ThenReturnDefaultError).
		Send()
	c.Require().NoError(err)
}
